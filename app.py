...
[data-testid="stMetric"] {
    ...
    box-shadow: 0 2px 12px rgba(80,50,10,0.07);
    transition: box-shadow 0.2s, transform 0.2s;
}
[data-testid="stMetric"]:hover {
    box-shadow: 0 6px 24px rgba(80,50,10,0.13);
...
.stButton > button[kind="primary"] {
    background: linear-gradient(135deg, #D4820A, #8B5200);
    color: white;
    border: none;
}
...
.stTextArea > div > div > textarea {
    ...
    border: 1px solid rgba(180,150,80,0.4) !important;
...
.alert-danger { background: #fef2f2; border: 1px solid #fecaca; color: #991b1b; border-left: 4px solid #ef4444; }
...
.badge-bon { background: #dbeafe; color: #1d4ed8; }
.badge-attention { background: #ffedd5; color: #c2410c; }
.badge-critique { background: #fee2e2; color: #b91c1c; }
.badge-intermissa { background: #fef9c3; color: #a16207; }
.badge-sahariensis { background: #f3e8ff; color: #7e22ce; }
...
STATUS_BADGES = {
    "Excellent": "badge-excellent", "Bon": "badge-bon",
    "Attention": "badge-attention", "Critique": "badge-critique"
}
RACE_BADGES = {
...
PROFIL_ICONS = {
    "Miel": "🍯", "Pollen": "🌼", "Gelée Royale": "👑", "Résistance": "🛡️"
}
PROFIL_COLORS = {
    "Miel": "#D4820A", "Pollen": "#F59E0B", "Gelée Royale": "#9B59B6", "Résistance": "#22C55E"
}
...
def metric_card(icon, value, label, trend="", trend_up=True):
    color = "#22c55e" if trend_up else "#ef4444"
    arrow = "↑" if trend_up else "↓"
    return f"""
    <div style="background:white;border-radius:16px;padding:20px 22px;border:1px solid rgba(180,150,80,0.2);
                box-shadow:0 2px 12px rgba(80,50,10,0.07);transition:all 0.2s;">
        <div style="font-size:26px;margin-bottom:8px">{icon}</div>
        <div style="font-family:'Playfair Display',serif;font-size:28px;font-weight:700;color:#1E1A0F">{value}</div>
        <div style="font-size:11px;text-transform:uppercase;letter-spacing:0.08em;color:#6B6040;font-weight:600;margin-top:2px">{label}</div>
        {"" if not trend else f'<div style="font-size:12px;color:{color};margin-top:6px">{arrow} {trend}</div>'}
    </div>"""
...
def ruche_card_html(r):
    sc = STATUS_COLORS.get(r["Statut"], "#6b7280")
    pi = PROFIL_ICONS.get(r["Profil_prod"], "🐝")
    pc = PROFIL_COLORS.get(r["Profil_prod"], "#D4820A")
    rb = RACE_BADGES.get(r["Race"], "badge-hybride")
    return f"""
    <div class="ruche-card">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px">
            <span class="ruche-id-tag">{r["ID"]}</span>
            <div style="display:flex;align-items:center;gap:8px">
                <span title="{r['Profil_prod']}" style="font-size:16px">{pi}</span>
                <div style="width:10px;height:10px;border-radius:50%;background:{sc};
                            box-shadow:0 0 0 3px {sc}22"></div>
            </div>
        </div>
        <div style="font-family:'Playfair Display',serif;font-size:17px;font-weight:700;
                    margin-bottom:2px">{r["Nom"]}</div>
        <div style="margin-bottom:12px">{badge(r["Race"], rb)}</div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
            <div style="background:#FFF8E6;border-radius:9px;padding:9px">
                <div style="font-weight:600;font-size:14px;color:#8B5200">{r["Poids_kg"]} kg</div>
                <div style="font-size:10px;text-transform:uppercase;letter-spacing:0.05em;color:#6B6040">Poids</div>
            </div>
            <div style="background:#FFF8E6;border-radius:9px;padding:9px">
                <div style="font-weight:600;font-size:14px;color:#8B5200">{r["Varroa_pct"]}%</div>
                <div style="font-size:10px;text-transform:uppercase;letter-spacing:0.05em;color:#6B6040">Varroa</div>
            </div>
            <div style="background:#FFF8E6;border-radius:9px;padding:9px">
                <div style="font-weight:600;font-size:14px;color:{pc}">{r["Miel_kg"]} kg</div>
        ...
        <div style="margin-top:10px;font-size:11px;color:#6B6040">📍 {r["Site"]}</div>
    </div>"""
...
def production_radar(ruche_row):
    ...
    fig = go.Figure(go.Scatterpolar(
        ...
        fill='toself',
        fillcolor='rgba(212,130,10,0.15)',
    ...
    fig.update_layout(
        polar=dict(
            ...
            radialaxis=dict(visible=True, range=[0, 100], tickfont=dict(size=9, color='#6B6040'),
                            gridcolor='rgba(180,150,80,0.2)', linecolor='rgba(180,150,80,0.2)'),
            angularaxis=dict(tickfont=dict(size=11, color='#4A3728'), gridcolor='rgba(180,150,80,0.15)')
        ),
        showlegend=False,
        height=300,
